...
<div class="p-8">
    <div class="max-w-5xl mx-auto">
        ...
        <form id="multaForm" method="POST" action="{{ $cliente->multas->first() ? route('simit-registros.update', $cliente->multas->first()->id) : '#' }}" class="bg-white rounded-lg shadow-lg p-8" data-store-url="{{ route('clientes.store') }}">
            @csrf
            @method('PUT')
            <input type="hidden" name="_method" id="formMethod" value="PUT">
            <input type="hidden" name="multa_id" id="multa_id" value="{{ $cliente->multas->first()->id ?? '' }}">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="nombre" id="nombre" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old('nombre', $cliente->nombre) }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="numero_documento" id="numero_documento" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old('numero_documento', $cliente->numero_documento) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <div id="multasContainer">
                    @foreach($cliente->multas as $index => $multa)
                        <div class="multa-item mb-6 p-6 border-2 border-gray-200 rounded-lg bg-gray-50">
                            <div class="flex justify-between items-center mb-4">
                                <h5 class="text-lg font-semibold text-gray-700">Multa #{{ $index + 1 }}</h5>
                                <button type="button" onclick="eliminarMulta(this)" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm {{ $cliente->multas->count() > 1 ? '' : 'hidden' }} eliminar-multa-btn">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="multas[{{ $index }}][placa]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old("multas.$index.placa", $multa->placa) }}">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="multas[{{ $index }}][departamento]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old("multas.$index.departamento", $multa->departamento) }}">
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Fecha *</label>
                                    <input type="text" name="multas[{{ $index }}][fecha]" required placeholder="dd/mm/aaaa" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old("multas.$index.fecha", is_string($multa->fecha) ? $multa->fecha : $multa->fecha->format('d/m/Y')) }}">
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Comparendo *</label>
                                    <input type="text" name="multas[{{ $index }}][comparendo]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old("multas.$index.comparendo", $multa->comparendo) }}">
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Estado de Pago *</label>
                                    <select name="multas[{{ $index }}][estado_pago]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                        <option value="pendiente" {{ old("multas.$index.estado_pago", $multa->estado_pago) == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                                        <option value="pagado" {{ old("multas.$index.estado_pago", $multa->estado_pago) == 'pagado' ? 'selected' : '' }}>Pagado</option>
                                        <option value="vencido" {{ old("multas.$index.estado_pago", $multa->estado_pago) == 'vencido' ? 'selected' : '' }}>Vencido</option>
                                    </select>
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Secretaría *</label>
                                    <input type="text" name="multas[{{ $index }}][secretaria]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old("multas.$index.secretaria", $multa->secretaria) }}">
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Código de Infracción *</label>
                                    <input type="text" name="multas[{{ $index }}][codigo_infraccion]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ old("multas.$index.codigo_infraccion", $multa->codigo_infraccion) }}">
                                </div>
                                
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Infracciones *</label>
                                    <input type="text" name="multas[{{ $index }}][infracciones]" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Descripción de la infracción" value="{{ old("multas.$index.infracciones", $multa->infracciones) }}">
                                </div>
                            </div>
                            ...
                            <div class="mt-6 border-t border-dashed border-gray-300 pt-4">
                                ...
                                <textarea
                                    ...
                                    rows="6"
                                    placeholder="Pega aquí el texto del comparendo correspondiente a esta multa"></textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="forma_pago" id="forma_pago" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" onchange="actualizarFormaPago()">
                                <option value="">Seleccione...</option>
                                <option value="pago_unico" {{ old('forma_pago', $cliente->forma_pago) == 'pago_unico' ? 'selected' : '' }}>Pago Único</option>
                                <option value="acuerdo_pago" {{ old('forma_pago', $cliente->forma_pago) == 'acuerdo_pago' ? 'selected' : '' }}>Acuerdo de Pago</option>
                            </select>
                        </div>
...
<script>
    // Inicializar contador de multas
